...
const Profile: React.FC<ProfileProps> = ({ user, onLogout }) => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploading(true);
    try {
      // محاكاة الرفع باستخدام FileReader وتخزين البيانات محلياً
      ...
      reader.onloadend = () => {
        ...
        // 2. تحديث المستخدم في LocalStorage
        const storedUser = localStorage.getItem('rafiq_user');
        if (storedUser) {
          const parsedUser = JSON.parse(storedUser);
          parsedUser.avatar = base64String;
          localStorage.setItem('rafiq_user', JSON.stringify(parsedUser));
        }
