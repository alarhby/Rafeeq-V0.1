...
import { ArrowRight, Camera, Upload, CheckCircle, ShieldCheck, AlertCircle, FileCheck, Info, FileText, Image as ImageIcon } from 'lucide-react';
...
const AddVehicle: React.FC<AddVehicleProps> = ({ user }) => {
  ...
  const simulateUpload = (docKey: keyof typeof uploadedDocs) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // هنا يتم إرسال الطلب وتغيير حالة المستخدم إلى PENDING في قاعدة البيانات
    alert('تم إرسال طلب إضافة السيارة ووثائق التوثيق بنجاح. سيتم مراجعة حسابك خلال 24 ساعة.');
    navigate('/vehicles');
  };
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 pb-20">
      <div className="flex items-center gap-4 mb-2">
        ...
        <div>
          <h2 className="text-2xl font-bold text-gray-800">إضافة سيارة وتوثيق حساب</h2>
          <p className="text-sm text-gray-500">أكمل البيانات لتصبح سائقاً معتمداً</p>
        </div>
      </div>
          ...
          <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-50 space-y-6 animate-in fade-in slide-in-from-left-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
                ...
                <div 
                  key={pic.key}
                  onClick={() => simulateUpload(pic.key)}
                  className={`aspect-video rounded-3xl border-2 border-dashed flex flex-col items-center justify-center cursor-pointer transition ${uploadedDocs[pic.key] ? 'bg-green-50 border-green-500 text-green-600' : 'bg-gray-50 border-gray-200 text-gray-400 hover:border-blue-400'}`}
                >
                  {uploadedDocs[pic.key] ? <CheckCircle size={32} /> : <Camera size={32} />}
        ...
        {step === 4 && (
          <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-50 space-y-6 animate-in fade-in slide-in-from-left-4">
            ...
            <div className="space-y-4">
              {[
                ...
                <div 
                  ...
                  className={`p-5 rounded-2xl border-2 transition flex items-center justify-between cursor-pointer ${uploadedDocs[doc.key] ? 'bg-green-50 border-green-500' : 'bg-white border-gray-100 hover:border-blue-200'}`}
                >
                  <div className="flex items-center gap-4">
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${uploadedDocs[doc.key] ? 'bg-green-200 text-green-700' : 'bg-gray-100 text-gray-400'}`}>
                      <Upload size={20} />
                    </div>
                    <div>
                  ...
                  {uploadedDocs[doc.key] && <CheckCircle size={20} className="text-green-600" />}
                </div>
              ))}
            </div>
            ...
            <div className="flex gap-3">
               ...
               <button 
                  type="submit" 
                  className="flex-1 bg-blue-600 text-white py-4 rounded-2xl font-bold shadow-xl shadow-blue-200 hover:bg-blue-700 transition"
                >
                  إرسال طلب التوثيق والمركبة
                </button>
            </div>
          </div>
        )}
